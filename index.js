...
function processSum(numberList, callback) {
  let sum = numberList.reduce((acc, val) => {
    return acc + val;
  }, 0);
  return callback(sum);
  /* CODE HERE */
...
function firstNamesAllCaps(runners) {
  const capsRunners = runners.map(function(name) {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const sameSizeShirts = runners.filter(function(item) {
    return item.shirt_size === tShirtSize;
  });
  return sameSizeShirts;
  /* CODE HERE */
}
...
function tallyUpDonations(runners) {
  const sumOfAllDonations = runners.reduce(function(accumulator, item) {
    return accumulator + item.donation;
  }, 0);

  return sumOfAllDonations;
}
 ...
 * etc
 */
function counterMakerWithLimit(maxValueCount) {
  var count = 0;
